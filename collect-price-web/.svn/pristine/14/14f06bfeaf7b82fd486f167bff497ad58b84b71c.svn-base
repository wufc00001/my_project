...
//        }
        ...
        if(StringUtils.isNotBlank(goodsName)&&goodsName.length()>200){
            return Mono.justOrEmpty(buildResult(Constants.FAIL_CODE, "品种名称长度不能超过200个字符"));
        }
        if(StringUtils.isNotBlank(goodsWeight)&&goodsWeight.length()>20){
            return Mono.justOrEmpty(buildResult(Constants.FAIL_CODE, "重量长度不能超过20个字符"));
        }
        if(StringUtils.isNotBlank(goodsDenomination)&&goodsDenomination.length()>20){
            return Mono.justOrEmpty(buildResult(Constants.FAIL_CODE, "面额长度不能超过20个字符"));
        }
        if(StringUtils.isNotBlank(maxCirculation)&&maxCirculation.length()>15){
            return Mono.justOrEmpty(buildResult(Constants.FAIL_CODE, "最大发行量长度不能超过15个字符"));
        }
        if(StringUtils.isNotBlank(actualCastingQuantity)&&actualCastingQuantity.length()>15){
            return Mono.justOrEmpty(buildResult(Constants.FAIL_CODE, "实铸量长度不能超过15个字符"));
        }
